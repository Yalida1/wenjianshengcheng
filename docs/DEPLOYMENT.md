...
使用 `docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --build` 作为参考编排。生产必须设置 `APP_ENV=production`、至少 32 字符随机密钥、独立强密码（包括至少 12 位的 `DEMO_ADMIN_PASSWORD`）、HTTPS、`SESSION_COOKIE_SECURE=true`、准确 CORS、持久卷、备份计划和外部监控。数据库、Redis、MinIO 不应暴露公网。
