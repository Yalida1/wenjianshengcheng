...
const TEMPLATES: Template[] = [
  ...
  },
  {
    ...
    variables: 41,
    source: "公司标准模板库",
    deprecatedDate: "2026-06-30",
    replacement: "货物类公开招标文件模板 V3.0",
    requiredVars: 29,
    availableVars: 26,
  },
];
...
const STATUS_META: Record<
  TemplateStatus,
  {
    label: string;
    cls: string;
    icon: "check-circle" | "alert" | "clock" | "x-circle" | "circle";
  }
> = {
  active: {
    label: "当前有效",
    cls: "bg-[#ECF8F2] text-[#116B46]",
    icon: "check-circle",
  },
  deprecated: {
    label: "已停用",
    cls: "bg-[#FEF1F2] text-[#A8323C]",
    icon: "x-circle",
  },
  expired: {
    label: "已过期",
...
function TemplateStatusBadge({ status }: { status: TemplateStatus }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[11px] font-medium ${m.cls}`}
    >
        ...
        <div className="h-[calc(100%-64px)] overflow-auto p-6">
          ...
          {template.status === "deprecated" && (
            <div className="mb-4 rounded-lg border border-[#FECDD0] bg-[#FEF1F2] p-3 text-sm">
              <p className="font-medium text-[#A8323C]">
                该模板已于 {template.deprecatedDate} 停用
              </p>
          ...
          <div className="mb-5">
            ...
            <div className="space-y-1">
              {CHAPTERS_PREVIEW.slice(0, template.chapters).map((ch, i) => (
                ...
                >
                  <span className="w-5 shrink-0 text-right font-mono text-[11px] text-slate-400">
                    {i + 1}.
                  </span>
                  {ch}
                </div>
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-4">
              <div className="mb-3 grid grid-cols-3 gap-3 text-center">
                {[
                  {
                    label: "必填变量",
                    v: template.requiredVars,
                    cls: "text-slate-800",
                  },
                  {
                    label: "已具备",
                    v: template.availableVars,
                    cls: "text-[#116B46]",
                  },
                  {
                    label: "待确认",
                    v: template.requiredVars - template.availableVars,
                    cls: "text-[#8B520B]",
                  },
                ].map((s) => (
                  <div key={s.label}>
              ...
              <div className="h-1.5 rounded-full bg-slate-200">
                <div
                  className="h-full rounded-full bg-[#2E5495]"
                  style={{
                    width: `${Math.round((template.availableVars / template.requiredVars) * 100)}%`,
                  }}
                />
              </div>
    ...
    <div className="mb-5 flex flex-wrap items-center gap-x-6 gap-y-2 rounded-xl border border-slate-200 bg-white px-5 py-3">
      {[
        ...
        <div key={k} className="flex items-center gap-2 text-[13px]">
          ...
          <span className={`font-medium ${k === "P0 字段" ? "text-[#116B46]" : "text-slate-700"}`}>
            {v}
          </span>
        </div>
      ))}
    ...
    <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 bg-white px-6 py-3 xl:px-8">
      ...
      {[
        ...
        <div key={f.label} className="flex items-center gap-1.5">
          ...
          <span className="rounded-md border border-[#DCE7F7] bg-[#F2F6FC] px-2 py-1 text-xs font-medium text-[#24457C]">
            {f.value}
          </span>
        </div>
      ))}
  ...
  return (
    <tr
      className={`border-b border-slate-100 transition-colors ${
        selected ? "bg-[#F2F6FC]" : selectable ? "hover:bg-[#FBFCFE]" : "opacity-70"
      }`}
    >
        ...
        <button
          ...
          className={`size-5 rounded-full border-2 flex items-center justify-center transition-colors ${
            selected
              ...
              : selectable
                ? "border-slate-400 hover:border-[#2E5495]"
      ...
      <td className="px-4 py-4">
        <div className="flex items-start gap-2 flex-wrap">
          <p className={`font-medium ${selectable ? "text-slate-800" : "text-slate-500"}`}>
            {t.name}
          </p>
          {selected && (
            <span className="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[11px] font-medium bg-[#2E5495] text-white">
        ...
        {t.mismatchReason && (
          <p className="mt-1 text-xs text-[#A8323C]">
            <Icon name="alert" size={11} /> {t.mismatchReason}
          </p>
        )}
        {t.status === "deprecated" && (
          <p className="mt-1 text-xs text-[#A8323C]">
            <Icon name="x-circle" size={11} /> 已停用于 {t.deprecatedDate}
            。替代：{t.replacement}
          </p>
        )}
      </td>
      ...
      <td className="px-4 py-4 text-[13px] text-slate-500 hidden xl:table-cell">
        {t.effectiveDate}
      </td>
          ...
          {selectable && (
            <button
              ...
              className={`h-8 rounded-lg border px-3 text-[12px] font-medium transition-colors ${
                selected
                  ? "border-[#2E5495] bg-[#F2F6FC] text-[#24457C]"
                  : "border-[#2E5495] text-[#2E5495] hover:bg-[#F2F6FC]"
              }`}
            >
              {selected ? "已选择" : "选择"}
            </button>
    ...
    <div className="flex flex-1 items-center justify-center py-16">
      <div className="max-w-md text-center">
        ...
        <p className="mt-2 text-sm text-slate-500 leading-6">
          当前条件：货物类 · 公开招标 · 设备采购及安装 · 某省通信有限公司
          <br />
          没有找到当前有效且完全匹配的模板。
        </p>
  ...
  return (
    <div className="mb-5 flex items-start gap-3 rounded-xl border border-[#DCE7F7] bg-[#F6F8FB] px-5 py-3">
      ...
      <div className="text-[13px] text-slate-600">
        ...
        文件阶段：招标文件 · 采购类型：货物类 · 采购方式：公开招标 · 业务类型：设备采购及安装 ·
        所属组织：某省通信有限公司 · 当前有效版本。
    ...
    <div className="flex flex-1 min-h-0 flex-col overflow-hidden">
      ...
      <div className="border-b border-slate-200 bg-white px-6 py-4 xl:px-8 shrink-0">
        ...
        <p className="mt-1 text-[13px] text-slate-500">
          请选择本次招标文件使用的正式模板。系统将锁定所选模板版本，并根据已确认的项目字段生成内容。
        </p>
      </div>
          ...
          <div className="mb-3 flex items-center gap-3 rounded-lg border border-[#DCE7F7] bg-[#F2F6FC] px-4 py-2">
            ...
            <span className="text-[13px] text-[#24457C]">
              ...
              <span className="ml-3 text-slate-500">
                · {selected.chapters} 章节 · {selected.variables} 个变量
              </span>
            </span>
          </div>
        )}
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            <span className="text-xs text-slate-400">
              已确认字段快照：<span className="font-mono">FS-20260905-001</span>
            </span>
            <button
