...
const VERSIONS: VersionRecord[] = [
  ...
  },
  {
    ...
    label: "审校后版本",
    author: "王明远",
    date: "2026-09-05 14:35",
    summary: "处理4.2节技术规格，补充附件说明",
  },
];
...
const HISTORICAL_DIFFS: HistoricalDiff[] = [
  {
    id: "h1",
    category: "采购需求及技术要求",
    current: "包含动环监控系统安装调试",
    historical: "未包含动环监控系统",
    action: "keep",
  },
  {
    id: "h2",
      ...
      <div className="shrink-0 border-b border-slate-200 bg-white px-6 py-3 xl:px-8">
        <div className="flex items-center gap-4">
          ...
          <span className="text-[13px] text-slate-500">
            共发现 {DIFFS.reduce((a, d) => a + (d.count || 0), 0)} 处变更
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          {DIFFS.map((d) => (
            <button
              ...
              className={`w-full px-4 py-3 text-left border-b border-slate-100 hover:bg-slate-50 ${
                selectedDiff === d.id ? "bg-[#F2F6FC]" : ""
              }`}
            >
              <p
                className={`text-[13px] font-medium ${
                  selectedDiff === d.id ? "text-[#24457C]" : "text-slate-700"
                }`}
              >
                {d.section}
              </p>
              <div className="flex items-center gap-2 mt-0.5">
                <span
                  className={`text-[11px] rounded px-1.5 py-0.5 ${
                    d.type === "modified"
                      ? "bg-[#FFF7E6] text-[#8B520B]"
                      : d.type === "added"
                        ? "bg-[#ECF8F2] text-[#116B46]"
        ...
        <div className="flex-1 min-w-0 overflow-y-auto p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <p className="text-[11px] font-semibold text-[#A8323C] mb-2 uppercase tracking-wide">
                修改前
              </p>
            ...
            <div>
              <p className="text-[11px] font-semibold text-[#116B46] mb-2 uppercase tracking-wide">
                修改后
              </p>
          ...
          <div className="flex gap-3">
            <button className="h-8 rounded-lg border border-slate-300 px-3 text-sm text-slate-600 hover:border-[#2E5495]">
              查看完整章节
            </button>
            <button className="h-8 rounded-lg border border-[#FFF7E6] bg-[#FFF7E6] px-3 text-sm text-[#8B520B] hover:bg-[#FDDBA0]">
              恢复此段（需二次确认）
            </button>
...
function HistoricalDocumentCompare({
  navigate,
  onUpload,
}: {
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-[13px] text-slate-500">
              当前文件：某省公司中心机房节能改造项目招标文件 V0.3 ←→ 对照文件：某省公司 2025
              年同类机房改造项目招标文件
            </p>
          </div>
          <button
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <aside className="w-[280px] shrink-0 border-r border-slate-200 bg-white overflow-y-auto">
          ...
          {HISTORICAL_DIFFS.map((d) => (
            ...
            >
              ...
              <span
                className={`text-[11px] ${
                  d.action === "review" ? "text-[#8B520B]" : "text-[#116B46]"
                }`}
              >
        ...
        <div className="flex-1 min-w-0 overflow-y-auto p-6">
          <div className="mb-4 rounded-lg border border-[#FFF7E6] bg-[#FFFBF0] p-3 text-[13px] text-[#8B520B]">
            ...
            历史人工文件只作为对照参考，不能自动成为当前项目的事实来源，不能一键覆盖已确认字段。
          </div>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <p className="text-[11px] font-semibold text-[#2E5495] mb-2 uppercase tracking-wide">
                当前文件
              </p>
          ...
          <div className="flex gap-3">
            <button className="h-8 rounded-lg border border-[#2E5495] bg-[#F2F6FC] px-3 text-sm text-[#2E5495] hover:bg-[#DCE7F7]">
              保留当前
            </button>
            {diff.fieldKey && (
              <button className="h-8 rounded-lg border border-slate-300 px-3 text-sm text-slate-600 hover:border-[#2E5495]">
            ...
            <button className="h-8 rounded-lg border border-[#116B46] bg-[#ECF8F2] px-3 text-sm text-[#116B46] hover:bg-[#D9F2E8]">
              标记已复核
            </button>
          </div>
          <button
...
function UploadHistoricalFile({
  ...
  const [uploadState, setUploadState] = useState<
    "idle" | "uploading" | "parsing" | "comparing" | "done" | "failed"
  >("idle");
        ...
        <div className="max-w-[640px] space-y-5">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <p className="text-[13px] text-slate-500 mb-3">
              支持 DOCX、PDF 格式。历史文件仅用于对照，不会覆盖当前字段和来源关系。
            </p>
            {uploadState === "idle" && (
              <>
                <div className="flex items-center gap-2 mb-3">
                  <input
                    ...
                    placeholder="某省公司 2025 年同类机房改造项目招标文件.docx"
                    className="flex-1 h-9 rounded-lg border border-slate-300 px-3 text-sm focus:border-[#2E5495] focus:outline-none"
                  />
                  <button className="h-9 rounded-lg border border-slate-300 px-3 text-sm text-slate-600 hover:border-[#2E5495]">
                    浏览
                  </button>
                </div>
                <button
...
export function DocumentComparePage({ navigate }: { navigate: (p: Page) => void }) {
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col overflow-hidden">
      ...
      <div className="flex shrink-0 border-b border-slate-200 bg-white px-6 xl:px-8">
        {[
          ...
          <button
            key={t.v}
            onClick={() => setMode(t.v)}
            className={`mr-1 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${
              mode === t.v
                ? "border-[#2E5495] text-[#2E5495]"
      ...
      {mode === "historical" && (
        <HistoricalDocumentCompare navigate={navigate} onUpload={() => setMode("upload")} />
      )}
      {mode === "upload" && (
        <UploadHistoricalFile navigate={navigate} onBack={() => setMode("historical")} />
      )}
    </div>
  );
}
