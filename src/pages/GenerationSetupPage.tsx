...
const INITIAL_CHECKS: PreflightItem[] = [
  ...
  {id:"c8",label:"模板必填变量具备可用值",      status:"checking",detail:"30 / 32 个必填变量具备映射值"},
...
function usePreflight(running: boolean) {
  ...
  useEffect(()=>{
    if(!running){ setChecks(INITIAL_CHECKS); setDone(false); return; }
...
function ConfirmGenerationDialog({onCancel,onConfirm}:{onCancel:()=>void;onConfirm:()=>void}) {
  return (
      ...
      <div className="w-full max-w-[500px] rounded-xl border border-slate-200 bg-white shadow-[0_18px_48px_rgba(15,23,42,.18)]">
        <div className="flex items-start justify-between border-b border-slate-200 px-6 py-5">
          <div>
            ...
            <p className="mt-1 text-[13px] text-slate-500">生成启动后，来源文件、字段快照和模板版本将被锁定，无法修改。</p>
          </div>
        ...
        <div className="p-6">
          ...
          <div className="rounded-lg border border-[#DCE7F7] bg-[#F6F8FB] px-4 py-3 text-[13px] text-[#24457C]">
            <Icon name="info" size={13}/>{" "}生成期间可切换至其他页面，系统将在后台继续生成，完成后通知您。
          </div>
...
function PolicyRow({label,desc,on}:{label:string;desc:string;on:boolean}) {
  return (
    <div className="flex items-center justify-between gap-4 py-3 border-b border-slate-100 last:border-0">
      ...
      <div className={`shrink-0 flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-medium ${on?"bg-[#ECF8F2] text-[#116B46]":"bg-slate-100 text-slate-500"}`}>
        <Icon name={on?"check":"close"} size={10}/>{on?"已开启":"已关闭"}
      </div>
    </div>
  );
}
...
export function GenerationSetupPage({navigate, onConfirmGenerate}:{navigate:(p:Page)=>void;onConfirmGenerate:()=>void}) {
    ...
    <div className="flex flex-1 min-h-0 flex-col overflow-hidden">
      ...
      <div className="border-b border-slate-200 bg-white px-6 py-4 xl:px-8 shrink-0">
        <nav className="flex items-center gap-2 text-xs text-slate-500 mb-3">
          ...
          <span>/</span>
          <button onClick={()=>navigate("template-selection")} className="hover:text-[#24457C]">选择模板</button>
          <span>/</span>
          <span className="font-medium text-slate-800">配置生成</span>
        </nav>
        <h1 className="text-xl font-semibold text-slate-800">配置生成</h1>
        <p className="mt-1 text-[13px] text-slate-500">确认来源信息、生成策略，然后启动预检并生成招标文件。</p>
      </div>
        ...
        <div className="flex gap-6 max-w-[1200px]">
          ...
          <div className="flex-[2] min-w-0 space-y-5">
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-5">
              ...
              <div className="space-y-2 text-[13px]">
                {[
                  {label:"来源文件",  value:"可研报告",  badge:"V1.3",  bCls:"bg-[#ECF8F2] text-[#116B46]",  icon:"file" as const,  note:"王明远 · 2026-09-04 定稿"},
                  {label:"字段快照",  value:"FS-20260905-001",  badge:"已锁定",  bCls:"bg-[#ECF8F2] text-[#116B46]",  icon:"lock" as const,  note:"42 个字段 · P0 × 12 全部已确认"},
                ].map(r=>(
                  <div key={r.label} className="flex items-center justify-between gap-3 rounded-lg border border-slate-100 px-4 py-3 bg-slate-50">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`rounded px-1.5 py-0.5 text-[11px] font-medium ${r.bCls}`}>{r.badge}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-5">
              ...
              <div className="flex items-start justify-between gap-4 rounded-lg border border-[#DCE7F7] bg-[#F6F8FB] px-4 py-3">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-slate-500">公司标准模板库 · 模板管理员 李敏 · 生效：2026-01-01</p>
                </div>
                <div className="shrink-0 flex items-center gap-2">
                  <span className="rounded px-1.5 py-0.5 text-[11px] font-medium bg-[#ECF8F2] text-[#116B46]">当前有效</span>
                  <span className="rounded px-1.5 py-0.5 text-[11px] font-medium bg-[#ECF8F2] text-[#116B46]">高度匹配</span>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-5">
                  <label className="flex items-center gap-2 text-[13px] text-slate-600">
                    <input type="checkbox" defaultChecked className="accent-[#2E5495]"/>同时生成 PDF 预览版
                  </label>
                  <label className="flex items-center gap-2 text-[13px] text-slate-600">
                    <input type="checkbox" className="accent-[#2E5495]"/>生成用于比对的 HTML 版本
                  </label>
                </div>
              </div>
            </section>
            ...
            {/* Chapter list */}
            <section className="rounded-xl border border-slate-200 bg-white p-5">
              <h2 className="text-sm font-semibold text-slate-800 mb-3">章节生成范围（8 个一级章节）</h2>
              <div className="grid grid-cols-2 gap-2">
                {CHAPTERS.map(ch=>(
                  <div key={ch} className="flex items-center gap-2 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2.5 text-[13px] text-slate-700">
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-5">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-xs text-slate-400 flex items-center gap-1"><Icon name="lock" size={11}/>管理员可在系统设置中调整</span>
              </div>
              <p className="text-xs text-slate-500 mb-4">以下策略由组织统一配置，本次生成不可修改。</p>
              {[
                {label:"严格字段映射",        desc:"仅使用已确认字段，不允许 AI 推断补全缺失的必填变量",     on:true},
                {label:"禁止自由创作段落",     desc:"章节描述必须来自字段或模板内置文本，不允许 AI 生成",    on:true},
                {label:"合规用词库过滤",       desc:'自动替换不合规表述，如「谈判」改为「投标」等',           on:true},
                {label:"一致性校验",          desc:"同一字段在不同章节出现时，保证表述一致",              on:true},
                {label:"版本水印",            desc:"在草稿页眉嵌入字段快照版本号和生成任务 ID",           on:true},
              ].map(p=><PolicyRow key={p.label} {...p}/>)}
            </section>
          </div>
          ...
          <div className="w-[300px] shrink-0 space-y-4">
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-5">
              ...
              {!preflightRunning && (
                <p className="mb-4 text-[13px] text-slate-500">点击"运行预检"验证所有条件后，即可启动生成。</p>
              )}
              <div className="space-y-2 mb-4">
                {checks.map(c=>(
                  <div key={c.id} className="flex items-start gap-2.5">
                    <div className={`mt-0.5 shrink-0 ${c.status==="passed"?"text-[#116B46]":c.status==="blocked"?"text-[#A8323C]":c.status==="checking"&&preflightRunning?"text-[#2E5495]":"text-slate-300"}`}>
                      ...
                      {c.status==="checking" && preflightRunning && (
                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="animate-spin">
                          <path d="M21 12a9 9 0 1 1-9-9"/><path d="M21 3v9h-9"/>
                        </svg>
                      )}
                      {c.status==="checking" && !preflightRunning && <Icon name="circle" size={15}/>}
                    </div>
                    <div>
                      <p className={`text-[13px] leading-snug ${c.status==="passed"?"text-slate-700":c.status==="blocked"?"text-[#A8323C]":"text-slate-400"}`}>{c.label}</p>
                      {c.detail && c.status==="passed" && <p className="text-[11px] text-slate-400 mt-0.5">{c.detail}</p>}
                    </div>
                  </div>
                ))}
              </div>
              ...
              {preflightRunning && !done && (
                <div className="flex items-center justify-center gap-2 text-[13px] text-[#2E5495] py-1">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="animate-spin"><path d="M21 12a9 9 0 1 1-9-9"/><path d="M21 3v9h-9"/></svg>
                  预检中，请稍候…
                </div>
              )}
              {done && (
                <div className="rounded-lg border border-[#C3E8D5] bg-[#ECF8F2] px-3 py-2.5 flex items-center gap-2 text-sm text-[#116B46] font-medium">
                  <Icon name="check-circle" size={15}/>全部 10 项预检通过
                </div>
              )}
            </section>
      ...
      {showConfirmDialog && (
        <ConfirmGenerationDialog
          ...
          onConfirm={()=>{setShowConfirmDialog(false);onConfirmGenerate();navigate("generation-progress");}}/>
      )}
    </div>
  );
}
