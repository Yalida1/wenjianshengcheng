      ...
      <div className="flex-1 min-h-0 overflow-y-auto bg-[#F6F8FB] px-6 py-8 xl:px-8">
        <div className="mx-auto max-w-[720px] grid grid-cols-2 gap-5">
          <button
            ...
            className={`text-left rounded-xl border-2 bg-white p-6 transition-all hover:shadow-md ${
              feasFinalized
                ? "border-[#2E5495] cursor-pointer"
          ...
          >
            ...
            <p className="text-[13px] text-slate-500 mb-4 leading-5">
              直接继承平台已定稿可研报告的字段和来源关系，自动提取招标字段。
            </p>
            {feasFinalized ? (
...
export function TenderFileUploadPage({ navigate }: { navigate: (p: Page) => void }) {
  const [files, setFiles] = useState([
    { name: "可行性研究报告_V1.3_终稿.docx", size: "2.4 MB", status: "ready" },
  ]);
        ...
        <div className="mx-auto max-w-[680px] space-y-5">
          <div
            ...
            className={`flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed py-12 transition-all ${
              dragging ? "border-[#2E5495] bg-[#F2F6FC]" : "border-slate-300 bg-white"
            }`}
          >
            ...
            <button className="h-9 rounded-lg border border-[#2E5495] px-5 text-sm font-medium text-[#2E5495] hover:bg-[#F2F6FC]">
              选择文件
            </button>
          </div>
          ...
          {files.length > 0 && (
            <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
              <div className="border-b border-slate-100 px-5 py-3 text-xs text-slate-500 font-semibold">
                已选文件
              </div>
              {files.map((f) => (
                <div key={f.name} className="flex items-center gap-4 px-5 py-4">
                  ...
                  <span className="rounded-full bg-[#ECF8F2] px-2.5 py-1 text-[11px] font-medium text-[#116B46]">
                    就绪
                  </span>
                  <button
                    onClick={() => setFiles([])}
            ...
            <div className="space-y-3">
              {steps.map((s, i) => (
                ...
                >
                  ...
                  {i === step && (
                    <svg
                      className="animate-spin ml-auto text-[#2E5495]"
                      width="14"
                      height="14"
                      viewBox="0 0 24 24"
                      fill="none"
...
const EXTRACTED = [
  {
    group: "项目基础信息",
    fields: ["项目名称", "项目编号", "建设单位", "建设地点", "建设规模"],
  },
  {
    group: "投资与预算",
    fields: ["项目总投资", "建安工程费", "设备购置费", "其他费用"],
  },
  {
    group: "技术指标",
    fields: ["设备数量", "冷量需求", "能效比要求", "安装周期"],
  },
  {
    group: "合规与资质",
    fields: ["资质要求", "项目负责人要求", "售后服务要求"],
  },
];
      ...
      <div className="border-b border-slate-200 bg-white px-6 py-4 xl:px-8 shrink-0">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-[13px] text-slate-500">
              共提取 42 个字段，38 个自动填充，4 个需要人工确认。
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <span className="rounded-full bg-[#ECF8F2] px-3 py-1 text-[12px] font-medium text-[#116B46]">
              解析成功
            </span>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-[800px] space-y-4">
            ...
            <div className="flex items-center gap-4 px-5 py-4">
              ...
              <div>
                ...
                <p className="text-[12px] text-slate-400">
                  字段快照 FS-20260905-001 · 2026-09-05 14:32
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-[#FFF0C0] bg-[#FFFBEB] p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-[13px] text-[#8B520B]">
                  以下 4 个字段需要人工确认
                </p>
          ...
          <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-4">
            <input
              ...
              className="h-4 w-4 rounded border-slate-300 text-[#2E5495]"
            />
          ...
          <div className="flex justify-end gap-3 pb-6">
            <button
              onClick={() => navigate("tender-file-upload")}
              className="h-10 rounded-lg border border-slate-300 px-5 text-sm text-slate-600 hover:border-[#2E5495]"
            >
              重新上传
            </button>
