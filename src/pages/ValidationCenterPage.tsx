...
const GROUPS: CheckGroup[] = [
  ...
  {id:"g2",name:"字段完整性",checks:[
    ...
    {id:"c2-4",label:"不存在未替换模板变量",status:"fail_p0",detail:"投标人须知第 2.2 节：{{ tender_number }}"},
  ]},
  {id:"g3",name:"字段一致性",checks:[
    {id:"c3-1",label:"项目名称全文一致",status:"pass"},
    {id:"c3-2",label:"建设单位名称全文一致",status:"pass"},
    {id:"c3-3",label:"招标预算全文一致",status:"fail_p0",detail:"招标公告：¥9,800,000；商务要求：¥12,800,000"},
  ]},
  {id:"g4",name:"金额与数值",checks:[
    {id:"c4-1",label:"数字金额与大写金额一致",status:"pass"},
    {id:"c4-2",label:"招标预算未错误引用可研总投资",status:"pass"},
    {id:"c4-3",label:"设备数量和单位全文一致",status:"pass"},
  ]},
  {id:"g5",name:"日期与期限",checks:[
    {id:"c5-1",label:"日期先后顺序合理",status:"pass"},
    {id:"c5-2",label:"项目总周期未等同本次交付周期",status:"fail_p1",detail:"请确认采购交付周期（90日）与项目总建设周期关系"},
  ]},
  {id:"g6",name:"采购范围",checks:[
    {id:"c6-1",label:"采购范围已确认且全文一致",status:"pass"},
    {id:"c6-2",label:"采购范围未超出已确认字段",status:"pass"},
  ]},
  {id:"g7",name:"技术参数",checks:[
    {id:"c7-1",label:"技术参数在正文与附件中一致",status:"pass"},
    {id:"c7-2",label:"无无来源技术指标",status:"pass"},
  ]},
  {id:"g8",name:"固定条款",checks:[
    {id:"c8-1",label:"固定模板条款未被未经授权修改",status:"pass"},
  ]},
  {id:"g9",name:"AI 内容审阅",checks:[
    {id:"c9-1",label:"关键 AI 内容已人工审阅",status:"fail_p1",detail:"采购需求及技术要求章节存在 3 段未审阅 AI 草稿"},
    {id:"c9-2",label:"AI 内容未引用虚构法规或数据",status:"pass"},
  ]},
  {id:"g10",name:"占位符",checks:[
    {id:"c10-1",label:"无未替换的模板变量",status:"fail_p0",detail:"{{ tender_number }}，位于投标人须知"},
  ]},
  {id:"g11",name:"文档格式",checks:[
    {id:"c11-1",label:"页眉、页脚、页码符合模板规范",status:"pass"},
    {id:"c11-2",label:"标题样式符合模板规范",status:"pass"},
    {id:"c11-3",label:"无未处理批注",status:"pass"},
  ]},
  {id:"g12",name:"来源可追溯性",checks:[
    {id:"c12-1",label:"关键内容可追溯到字段或来源材料",status:"pass"},
    {id:"c12-2",label:"关键字段可追溯率",status:"pass",detail:"100%"},
  ]},
];
...
function statusSev(s:Check["status"]) {
  if(s==="fail_p0") return <span className="rounded px-1 py-0.5 bg-[#FEF1F2] text-[#A8323C] text-[10px] font-bold">P0</span>;
  if(s==="fail_p1") return <span className="rounded px-1 py-0.5 bg-[#FFF7E6] text-[#8B520B] text-[10px] font-bold">P1</span>;
  if(s==="fail_p2") return <span className="rounded px-1 py-0.5 bg-slate-100 text-slate-500 text-[10px] font-bold">P2</span>;
  return null;
}
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <div className="mt-3 grid grid-cols-3 gap-4 text-center">
            <div><p className="text-2xl font-bold text-[#A8323C]">2</p><p className="text-[11px] text-[#A8323C]">P0 阻断问题</p></div>
            <div><p className="text-2xl font-bold text-[#8B520B]">2</p><p className="text-[11px] text-[#8B520B]">P1 重要问题</p></div>
            <div><p className="text-2xl font-bold text-slate-500">2</p><p className="text-[11px] text-slate-500">P2 建议</p></div>
          </div>
          <p className="mt-3 text-sm text-[#A8323C]">请先解决所有 P0 问题。P1 问题需要处理或明确确认后才能定稿。</p>
        </div>
      </div>
    </div>
  );
        ...
        <div>
          ...
          <div className="mt-3 grid grid-cols-4 gap-3 text-center text-[12px]">
            {[
              ...
              <div key={s.label}><p className={`text-base font-bold ${s.cls}`}>{s.v}</p><p className="text-[10px] text-slate-400">{s.label}</p></div>
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="mb-6 rounded-xl border border-slate-200 bg-white p-5">
      <div className="grid grid-cols-4 gap-4 text-center">
        <div><p className="text-2xl font-bold text-[#A8323C]">2</p><p className="text-[11px] text-slate-500">P0 阻断</p></div>
        <div><p className="text-2xl font-bold text-[#8B520B]">2</p><p className="text-[11px] text-slate-500">P1 重要</p></div>
        <div><p className="text-2xl font-bold text-slate-500">2</p><p className="text-[11px] text-slate-500">P2 建议</p></div>
        <div><p className="text-2xl font-bold text-[#116B46]">0</p><p className="text-[11px] text-slate-500">已解决</p></div>
      </div>
      <div className="mt-3 text-[13px] text-slate-600">
        校验时间：2026-09-05 14:26 · 来源：可研 V1.3 · 字段快照：FS-20260905-001 · 模板：货物类公开招标文件模板 V3.0
      </div>
    </div>
  );
}
...
export function ValidationCenterPage({navigate}:{navigate:(p:Page)=>void}) {
  ...
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(new Set(["g2","g3","g5","g9","g10"]));
    ...
    <div className="flex flex-1 min-h-0 flex-col overflow-hidden">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto bg-[#F6F8FB] px-6 py-6 xl:px-8">
        ...
        <div className="mb-5 flex gap-3">
          <button className="h-10 rounded-lg bg-[#C2414B] px-5 text-sm font-medium text-white hover:bg-[#A8323C]">处理阻断问题</button>
          <button onClick={()=>navigate("document-preview")} className="h-10 rounded-lg border border-slate-300 px-5 text-sm text-slate-600 hover:border-[#2E5495]">返回文档</button>
        </div>
                ...
                {expanded&&(
                  <div className="border-t border-slate-100">
                    {group.checks.map(check=>(
                      <div key={check.id} className={`flex items-start gap-3 px-5 py-3 border-b border-slate-50 last:border-0 ${check.status!=="pass"&&check.status!=="na"?"bg-[#FAFAFA]":""}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {check.detail&&<p className={`text-[12px] mt-0.5 ${check.status!=="pass"?"text-[#A8323C]":"text-slate-400"}`}>{check.detail}</p>}
                        </div>
                        {check.status!=="pass"&&check.status!=="na"&&(
                          <button className="shrink-0 h-6 rounded border border-slate-300 px-2 text-[11px] text-slate-600 hover:border-[#2E5495]">定位</button>
                        )}
                      </div>
                    ))}
                  </div>
